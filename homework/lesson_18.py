...
# Потому что у разного класса Dog или Cat есть метод make_sound и он работает по разному в каждом классе
# Этот метод находится внекласса потому что к нему могут обращаться и другие классы
...
# object = Animal()
# ошибка, нельзя создать объект напрямую, потому что animal это абстрактный класс и их не создают в объекты напрямую,
# потому что в абстрактном классе не чего не возвращается после pass
